module scaiev_fu import ariane_pkg::*; #(
    ...
    always @(posedge clk_i) begin
        if (!rst_ni) begin
            ...
            if (sv_valid_i) begin
                execute_pending_r <= execute_pending_r || !sv_valid_o;
            end
            else if (sv_valid_o) begin
                execute_pending_r <= 1'b0;
            end
    ...
    //Set non-ready if instruction present, i.e. both:
    //- comb/arrival: no instruction arriving, or instruction is writing back immediately
    //- multi-cycle: no instruction pending, or instruction is writing back
    //(ready is evaluated one stage back, so sv_valid_i must be factored in)
    assign sv_ready_o = (!sv_valid_i
            || (!execute_pending_r && sv_valid_o && !scaiev_execute_semicoupled_reenq)
               //If a hidden instruction is reentering,
            || scaiev_execute_semicoupled_deq)
        && (!execute_pending_r || sv_valid_o);
