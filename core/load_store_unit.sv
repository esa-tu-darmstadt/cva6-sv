...
#(
  ...
  `ifdef SCAIEV_MEM
    ...
    shift_reg #(
        .dtype(logic [$bits(st_valid) + $bits(st_trans_id) + $bits(st_result) + $bits(st_ex) + 1 - 1:0]),
        .Depth(CVA6Cfg.NrStorePipeRegs)
    ) i_pipe_reg_store (
        .clk_i,
        .rst_ni,
        .d_i({st_valid, st_trans_id, st_result, st_ex, st_has_trans_id}),
        .d_o({store_valid_o, store_trans_id_o, store_result_o, store_exception_o, store_has_trans_id_o})
    );
  `else
    shift_reg #(
  ...
  assign lsu_req_i = (scaiev_execute_wrMem_valid || scaiev_execute_rdMem_valid) ? {
    ...
    memOp,
    scaiev_execute_mem_has_trans_id,
  ...
  `ifdef SCAIEV_MEM
  logic scaiev_execute_mem_ready_;
  assign scaiev_execute_mem_ready = scaiev_execute_mem_ready_;
  `endif
  ...
  lsu_bypass #(
      ...
      .lsu_req_i      (lsu_req_i),
      .lsu_req_valid_i(lsu_valid_i `ifdef SCAIEV_MEM | ((scaiev_execute_rdMem_valid | scaiev_execute_wrMem_valid) & scaiev_execute_mem_ready_) `endif ),
      .pop_ld_i       (pop_ld),
      .pop_st_i       (pop_st),
      ...
      .ready_o   (lsu_ready_o),
      `ifdef SCAIEV_MEM
      .scaiev_execute_mem_ready(scaiev_execute_mem_ready_),
      `endif
      .*
  );
