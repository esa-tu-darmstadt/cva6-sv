...
#(
    ...
    output logic [63:0] scaiev_fetch_PC,
  ...
  `endif
  `ifdef SCAIEV_ENABLE
  assign scaiev_realign_rdInstr = instruction_valid[0] ? instr[0] : 0;
  ...
  //assign scaiev_preFetch_PC = npc_d;
  //assign scaiev_preFetch_isFlushing = preFetch_isFlushing;
  //assign scaiev_preFetch_isStalling = preFetch_isStalling;
  `endif
  ...
  always_comb begin
    ...
    //`ifdef SCAIEV_ZOL
    //  if(scaiev_realign_isZOLJump) predict_address = scaiev_realign_jumpAddr;
    //`endif
    // TODO: instr_queue should take the predicted PC into pc_j[1],
    //    i.e. it should register as a CF (without unintended side-effects)
    //`ifdef SCAIEV_ZOL
    //if (scaiev_fetch_wrPCValid) begin
    //  cf_type[i] = ariane_pkg::ZOL;
    //  predict_address = scaiev_fetch_wrPC;
    //end
    //if (scaiev_preFetch_wrPCValid) begin
    //  $error("Unhandled: scaiev_preFetch_wrPCValid");
    //end
    //`endif
  end
  // or reduce struct
  ...
  // Cache interface
  assign icache_dreq_o.req = instr_queue_ready `ifdef SCAIEV_ENABLE & ~scaiev_fetch_stall `endif ;
  assign if_ready = icache_dreq_i.ready & instr_queue_ready `ifdef SCAIEV_ENABLE & ~scaiev_fetch_stall `endif ;
  ...
  //`ifdef SCAIEV_ZOL
  //assign icache_dreq_o.kill_s2 = icache_dreq_o.kill_s1 | (bp_valid && !scaiev_realign_isZOLJump);
  //`else
  ...
  assign scaiev_fetch_isStalling = replay || !icache_dreq_i.ready || !instr_queue_ready;
  ...
  always_comb begin : npc_select
    ...
    if (scaiev_fetch_wrPCValid) begin
       npc_d = (instr_queue_ready && !flush_i && !(replay && scaiev_fetch_ignoreReplay))
               ? scaiev_fetch_wrPC + 4 : scaiev_fetch_wrPC;
