...
#(
    ...
    `ifdef SCAIEV_ENABLE
    input  fu_data_t [SUPERSCALAR:0] sv_fu_data_i,
    input  logic [SUPERSCALAR:0] sv_valid_i,
    ...
    output [CVA6Cfg.XLEN-1:0]                 sv_result_o,
  ...
  logic [SUPERSCALAR:0] sv_valid_hold;
  always_ff @(posedge clk_i) begin
    if (!rst_ni) begin
      sv_valid_hold <= '0;
    end
    else begin
      sv_valid_hold[0] <= ((sv_valid_i[0] || sv_valid_hold[0]) && !sv_ready_o);
      if (SUPERSCALAR) begin
        sv_valid_hold[1] <= ((sv_valid_i[1] || sv_valid_hold[1]) && !sv_ready_o);
      end
    end
  end
  always_comb begin
    ...
    if (SUPERSCALAR) begin
      if (sv_valid_i[1] || sv_valid_hold[1]) begin
        sv_fu_data = sv_fu_data_i[1];
      end
    end
  end
  scaiev_fu #(
  ...
  load_store_unit #(
      ...
      .load_exception_o,
      .store_trans_id_o,
      `ifdef SCAIEV_MEM .store_has_trans_id_o, `endif
      .store_result_o,
      .store_valid_o,
      .store_exception_o,
