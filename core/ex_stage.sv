...
#(
    ...
    output logic lsu_commit_speculative_empty_o,
    ...
    `ifdef SCAIEV_ENABLE
    input  fu_data_t [scaiev_config::NrFUIssuePorts-1:0]   sv_fu_data_i,
    input  logic [scaiev_config::NrFUIssuePorts-1:0]       sv_valid_i,
    output logic [scaiev_config::NrFUIssuePorts-1:0]       sv_ready_o,
    input  logic [scaiev_config::NrFUIssuePorts-1:0][31:0] sv_off_instr_i,
  ...
  branch_unit #(
      ...
      .resolve_branch_o,
      `ifdef SCAIEV_BRANCH
      .scaiev_execute_isBranch,
      .scaiev_execute_branch_isTaken,
      `endif
      `ifdef SCAIEV_JUMP
      .scaiev_execute_isJump,
      .scaiev_execute_jumpAddress,
      `endif
  ...
  `ifdef SCAIEV_ENABLE
  fu_data_t sv_fu_data;
  logic [31:0] sv_off_instr;
  logic sv_valid;
  logic [CVA6Cfg.NrIssuePorts-1:0] sv_valid_hold;
  always_ff @(posedge clk_i) begin
    if (!rst_ni) begin
      sv_valid_hold <= '0;
    end
    else begin
      sv_valid_hold[0] <= ((sv_valid_i[0] || sv_valid_hold[0]) && !sv_ready_o[0]);
      //if (CVA6Cfg.SuperscalarEn) begin
      //  sv_valid_hold[1] <= ((sv_valid_i[1] || sv_valid_hold[1]) && !sv_ready_o[1]);
      //end
    end
  end
  always_comb begin
    sv_fu_data = sv_fu_data_i[0];
    sv_off_instr = sv_off_instr_i[0];
    sv_valid = sv_valid_i[0];
  end
  if (scaiev_config::NrFUIssuePorts < 1) begin : staticassert_oneFUIssuePort_low
    $error("scaiev_config::NrFUIssuePorts must be at least 1");
  end
  else if (scaiev_config::NrFUIssuePorts > 1) begin : staticassert_oneFUIssuePort_high
    $error("scaiev_config::NrFUIssuePorts must not exceed 1 (limitation of current implementation)");
  end
  scaiev_fu #(
      .CVA6Cfg  (CVA6Cfg),
      .exception_t(exception_t),
      ...
      .clk_i,
      .rst_ni,
      .fu_data_i(sv_fu_data),
      .sv_valid_i(sv_valid),
      .sv_ready_o(sv_ready_o[0]),
      .sv_off_instr_i(sv_off_instr),
      .sv_trans_id_o,
      .sv_exception_o,
      .sv_result_o,
      .sv_valid_o,
      ...
      .scaiev_execute_rdInstr,
      .scaiev_execute_rdRS1,
      .scaiev_execute_rdRS2,
      .scaiev_execute_trans_id_o,

      .scaiev_execute_isValid,
      .scaiev_execute_isStalling,
      .scaiev_execute_stall,

      .scaiev_execute_wrRD_valid,
      .scaiev_execute_hasWriteback,
      `ifdef SCAIEV_BRANCH
      .scaiev_execute_isBranch,
      `endif
      `ifdef SCAIEV_JUMP
      ...
      `endif
      .scaiev_execute_wrRD,
      .scaiev_execute_trans_id_i,

      .scaiev_execute_semicoupled_deq,
      .scaiev_execute_semicoupled_reenq
  );

  assign scaiev_execute_lsuvalid = |lsu_valid_i;
  `endif
  ...
  load_store_unit #(
      ...
      .load_trans_id_o,
      `ifdef SCAIEV_MEM .load_has_trans_id_o, `endif
      .load_result_o,
      .load_valid_o,
      .load_exception_o,
      .store_trans_id_o,
      `ifdef SCAIEV_MEM .store_has_trans_id_o, .store_suppress_wb_o, `endif
      .store_result_o,
      .store_valid_o,
      .store_exception_o,
      .commit_i              (lsu_commit_i),
      .commit_ready_o        (lsu_commit_ready_o),
      .commit_speculative_empty_o (lsu_commit_speculative_empty_o),
      .commit_tran_id_i,
      ...
      .icache_areq_o,
      `ifdef SCAIEV_MEM
      .scaiev_execute_rdMem_result,
      .scaiev_execute_rdMem_result_valid,
      .scaiev_execute_rdMem_result_trans_id,
      .scaiev_execute_rdMem_result_has_trans_id,
      .scaiev_execute_mem_ready,
      .scaiev_execute_rdMem_valid,
      .scaiev_execute_memAddr,
      .scaiev_execute_memSize,
      .scaiev_execute_memAddr_valid,
      .scaiev_execute_wrMem_valid,
      .scaiev_execute_wrMem,
      .scaiev_execute_wrMem_suppress_wb,
